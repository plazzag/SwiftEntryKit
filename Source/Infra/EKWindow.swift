...
class EKWindow: UIWindow {
    ...
    init(with rootVC: UIViewController) {
        if #available(iOS 13.0, *) {
            ...
            if let scene = scenes.filter({$0.activationState == .foregroundActive}).first as? UIWindowScene {
                super.init(windowScene: scene)
            } else if let scene = scenes.filter({$0.activationState == .foregroundInactive}).first as? UIWindowScene {
                super.init(windowScene: scene)
            } else if let scene = scenes.first(where: { $0 is UIWindowScene } ) as? UIWindowScene {
                super.init(windowScene: scene)
            } else {
